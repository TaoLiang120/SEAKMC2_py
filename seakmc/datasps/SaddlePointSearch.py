...
def saddlepoint_search(thiscolor, istep, thissett, idav, thisAV, local_coords, thisSOPs, dynmatAV, SNC, CalPref,
                       ...
                       df_delete_SPs, DFWriter, object_dict, Precursor=False, insituGSPS=False):
    ...
    for idsps in range(thisnspsearch):
        ...
        if thissett.spsearch["Method"].upper() == "DIMER":
            ...
            thisspsearch = Dimer(idav, idsps, thisAVd, thissett, thiscolor, force_evaluator, SNC=SNC, dmAV=dynmatAV,
                                 pre_disps=thispredisp, apply_mass=thissett.spsearch["ApplyMass"],
        ...
        if thisspsearch.ISVALID:
            ...
            if isDup:
                ...
                if thissett.saddle_point["ValidSPs"]["RealtimeDelete"]:
                    df_delete_this, delete_this = thisSPS.realtime_validate_thisSP(thissp, insituGSPS=insituGSPS)
                    if delete_this:
                        df_delete_SPs = preSPS.update_df_delete_SPs(df_delete_SPs, df_delete_this, DFWriter)
                        thissp.ISVALID = False
            if thissp.ISVALID:
